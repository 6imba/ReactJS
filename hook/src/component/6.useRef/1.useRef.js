...
const UseRefComponent_1 = () => {
    ...
    useEffect(() => {
        ...
        console.log("Page render before", renderCounter.current ," times.")
        renderCounter.current += 1; //mutation and persist values between renders.
        console.log("Page render for next ernder", renderCounter)
    });
    return (
      <>
        <input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} />
...
// Finally,
    ...
    // - useRef() only returns one item. It returns an Object with one property called current. // {current: 0}
...
// You can say useRef is like a useState that maintains kind of state variable but doesn't re-render when its mutates.

// like useCallback set reference to function.
// like useRef set reference to variable.